...
public String generateEpisode(String chapterId, String dialog, boolean jsonMode) {
    ...
    @Override
    public String correctAnswerWithContext(String question, String answer, String template, String groupType, String correctionStyle) {
        // 1. 시스템 메시지 - 전문적인 자서전 편집자 역할
        String systemPrompt = """
            당신은 자서전 작성을 도와주는 전문 편집자입니다. 사용자의 답변을 질문의 맥락에 맞게 자연스럽고 감동적으로 교정해주세요.
            
            다음과 같은 원칙을 따라주세요:
            1. 질문의 의도와 답변의 내용이 잘 연결되도록 합니다
            2. 개인적인 경험과 감정이 생생하게 전달되도록 문체를 개선합니다
            3. 자서전 독자가 몰입할 수 있도록 스토리텔링 요소를 강화합니다
            4. 원본의 핵심 내용과 의미는 보존하면서 표현만 개선합니다
            
            응답은 반드시 JSON 형식으로 제공해주세요.
            """;
        
        // 2. 사용자 메시지 - 템플릿과 그룹 타입에 따른 맞춤형 프롬프트
        String templateContext = getTemplateContext(template);
        String groupTypeContext = getGroupTypeContext(groupType);
        String styleGuide = getCorrectionStyleGuide(correctionStyle);
        ...
        String userPrompt = String.format("""
            ...
            **요청사항:**
            ...
            위 정보를 바탕으로 답변을 교정하고, 다음 JSON 형식으로 응답해주세요:
            {
                "originalAnswer": "원본 답변",
                "correctedAnswer": "교정된 답변",
                "correctionReason": "교정 사유 및 개선점",
                "suggestedFollowUpQuestion": "추천 추가 질문 (선택적)"
            }
            """, 
            template, templateContext, groupType, groupTypeContext, correctionStyle,
            question, answer, styleGuide
        );
        
        // 3. ChatCompletion 요청
        ChatMessage system = ChatMessage.of("system", systemPrompt);
        ChatMessage user = ChatMessage.of("user", userPrompt);
    ...
    private String getTemplateContext(String template) {
        return switch (template) {
            case "INTRO" -> "소개 및 기본 정보";
            case "STORY" -> "주요 이야기 및 경험";
            case "REFLECTION" -> "회상 및 성찰";
            case "FUTURE" -> "미래 비전 및 유산";
            default -> "일반적인 내용";
        };
    }
    
    private String getGroupTypeContext(String groupType) {
        return switch (groupType) {
            case "FAMILY" -> "가족 관계와 유대감 중심";
            case "FRIENDS" -> "우정과 동료애 중심";
            case "COUPLE" -> "연인 관계와 로맨스 중심";
            case "TEAM" -> "팀워크와 협력 중심";
            case "OTHER" -> "다양한 관계와 경험";
            default -> "일반적인 관계";
        };
    }
    
    private String getCorrectionStyleGuide(String correctionStyle) {
        return switch (correctionStyle) {
            case "FORMAL" -> "격식 있고 품격 있는 문체로 교정해주세요. 존댓말을 사용하고 문학적 표현을 활용하세요.";
            case "CASUAL" -> "자연스럽고 친근한 문체로 교정해주세요. 일상적인 표현을 사용하되 감정이 잘 전달되도록 하세요.";
            case "LITERARY" -> "문학적이고 서정적인 문체로 교정해주세요. 은유와 비유를 활용하여 아름다운 표현으로 바꿔주세요.";
            case "CONCISE" -> "간결하고 명확한 문체로 교정해주세요. 핵심 내용을 압축적으로 표현하되 임팩트는 살려주세요.";
            default -> "읽기 쉽고 자연스러운 문체로 교정해주세요.";
        };
    }
