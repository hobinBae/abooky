...
@Getter
// 논리 삭제 적용
@SQLDelete(sql = "UPDATE group_book SET deleted_at = CURRENT_TIMESTAMP WHERE group_book_id = ?")
...
public class GroupBook {
    ...
    @JoinColumn(
            name = "member_id",
            nullable = false,
            foreignKey = @ForeignKey(name = "fk_group_book_member")
    )
    ...
    @JoinColumn(
            name = "group_id",
            nullable = false,
            foreignKey = @ForeignKey(name = "fk_group_book_group")
    )
    private Group group;
