...
public class CommunityBookController {
    ...
    @Operation(summary = "내가 작성한 커뮤니티 책 목록 조회", description = "내가 작성한 커뮤니티 책 목록을 조회합니다")
    @GetMapping()
    public ResponseEntity<ApiResponse<CommunityBookListResponse>> getMyCommunityBooks(
            @AuthenticationPrincipal CustomUserDetails userDetails,
            HttpServletRequest httpRequest,
            @PageableDefault(size = 10, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable,
            @RequestParam(required = false) Long categoryId,
        ...
        Long userId = userDetails.getMemberId();
        CommunityBookListResponse response = communityBookService.getMemberCommunityBooks(
                userId, userId, pageable, categoryId, bookType, sortBy);

        return ResponseEntity.status(HttpStatus.OK)
                .body(ApiResponse.of(HttpStatus.OK, "내가 작성한 커뮤니티 책 목록 조회 성공", response, httpRequest.getRequestURI()));
    }
    ...
    @Operation(summary = "커뮤니티 책 평점 조회", description = "해당 커뮤니티 책에 매긴 평점을 조회합니다")
    @GetMapping("/ratings/{communityBookId}")
    public ResponseEntity<ApiResponse<CommunityBookRatingResponse>> getRating(
            @Parameter(description = "커뮤니티 북 ID", required = true)
            @PathVariable Long communityBookId,
            @AuthenticationPrincipal CustomUserDetails userDetails,
            HttpServletRequest httpRequest) {

        CommunityBookRatingResponse response = communityBookService.getAverageRating(communityBookId);

        return ResponseEntity.status(HttpStatus.OK)
                .body(ApiResponse.of(HttpStatus.OK, "책 평점 조회 성공", response, httpRequest.getRequestURI()));
    }

    @Operation(summary = "북마크한 커뮤니티 책 목록 조회", description = "사용자가 북마크한 커뮤니티 책 목록을 조회합니다")
    @GetMapping("/bookmarks")
    public ResponseEntity<ApiResponse<CommunityBookListResponse>> getBookmarkedCommunityBooks(
            @AuthenticationPrincipal CustomUserDetails userDetails,
            HttpServletRequest httpRequest,
            @PageableDefault(size = 10, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable) {
        
        Long memberId = userDetails.getMemberId();
        CommunityBookListResponse response = communityBookService.getBookmarkedCommunityBooks(memberId, pageable);
