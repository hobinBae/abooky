...
async function deleteBook() {
  ...
  if (result) {
    try {
      ...
      await customAlert.value.showAlert({ title: '성공', message: '책이 삭제되었습니다.' });
...
async function addComment() {
  ...
  try {
    ...
    customAlert.value?.showAlert({ title: '오류', message: '댓글을 작성하지 못했습니다.' });
  }
}

async function deleteComment(commentId: number) {
  if (!customAlert.value) return;
  const result = await customAlert.value.showConfirm({
    title: '댓글 삭제',
  ...
  if (result) {
    try {
      ...
      customAlert.value?.showAlert({ title: '오류', message: '댓글을 삭제하지 못했습니다.' });
    }
  }
}
...
.author-controls { display: flex; gap: 6px; width: 100%; padding: 30px 8px; box-sizing: border-box; min-height: 85px; }
