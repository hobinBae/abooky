<template>
  <div class="intro-container">
    <ThreeScene ref="threeSceneRef" @background-loaded="onBackgroundLoaded" @loaded="onSceneLoaded"
      @hotspot="onHotspotActivated" @yard-animation-finished="onYardAnimationFinished" />
...
.nav-text {
  font-family: 'SSRockRegular', sans-serif;
  /* 새로 추가한 폰트 적용 */
  font-size: 4.5rem;
  /* 폰트 크기 조정 */
  font-weight: normal;
  /* ttf 폰트는 bold 대신 normal로 설정 */
  text-shadow: 1px 1px 3px rgba(0, 0, 0, 0.5);
  color: rgb(255, 255, 255);
}
...
.action-button-container {
  position: absolute;
  top: calc(20% + 15rem + 2rem);
  /* hotspot-title-container의 top(20%) + title 높이 + 여백 */
  left: 50%;
  transform: translateX(-50%);
  z-index: 100;
}
...
@media (max-width: 768px) {
  ...
  .hotspot-ui-container {
    /* flex-direction: column; 제거 -> 가로 정렬 유지 */
    flex-direction: row;
    justify-content: space-between;
    /* 요소들을 양쪽 끝으로 분산 */
    align-items: flex-end;
  ...
  .nav-control {
    /* flex-direction: column; 제거 -> 화살표와 텍스트를 가로로 유지 */
    flex-direction: row;
    align-items: center;
    /* 수직 중앙 정렬 */
    margin-bottom: 0;
  ...
  .hotspot-actions {
    /* 세로 정렬을 위해 추가했던 속성들 모두 제거 또는 초기화 */
    position: absolute;
    bottom: 5%;
    left: 50%;
    transform: translateX(-50%);
