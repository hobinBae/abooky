  ...
  <div class="auth-page my-page">
    ...
    <div class="auth-wrapper my-page-wrapper">
      ...
      <div class="auth-container my-page-container">
        ...
        <section class="content-section">
          <h3 class="section-title">좋아요한 책</h3>
          <div v-if="paginatedLikedBooks.length > 0" class="book-list-grid">
            <router-link v-for="book in paginatedLikedBooks" :key="book.communityBookId" :to="`/bookstore/book/${book.communityBookId}`" class="book-cover-link">
              <div class="book-cover-image"
                :style="{ backgroundImage: `url(${book.coverImageUrl || 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?q=80&w=1974'})` }">
                <div class="list-title-box">
                  <p class="list-title-box-title">{{ book.title }}</p>
          ...
          <p v-else class="no-content-message">아직 좋아요한 책이 없습니다.</p>
...
const likedBooks = ref<CommunityBook[]>([]);
...
onMounted(async () => {
  ...
  try {
    const bookmarkedResponse = await communityService.getBookmarkedCommunityBooks({ page: 0, size: 100 });
    bookmarkedBooks.value = bookmarkedResponse.content;
  } catch (error) {
    console.error('북마크한 책 목록을 불러오는데 실패했습니다:', error);
  }
  try {
    const likedResponse = await communityService.getLikedCommunityBooks({ page: 0, size: 100 });
    likedBooks.value = likedResponse.content;
  } catch (error) {
    console.error('좋아요한 책 목록을 불러오는데 실패했습니다:', error);
  }
});
</script>
