          ...
          <div class="form-section">
            ...
            <div class="profile-pic-section">
              ...
              <div class="pic-actions">
                ...
                <button type="button" @click="triggerFileUpload" class="btn btn-secondary">사진 업로드</button>
...
async function resetProfilePic() {
  if (!confirm('프로필 사진을 기본 이미지로 변경하시겠습니까? 이 작업은 즉시 저장됩니다.')) {
    return;
  }
  isSaving.value = true;
  try {
    // 기본 프로필 이미지를 가져와 File 객체로 변환
    const response = await fetch('/images/profile.png');
    const blob = await response.blob();
    const defaultProfileImage = new File([blob], 'profile.png', { type: 'image/png' });

    // FormData에 담아 서버로 전송
    const formData = new FormData();
    formData.append('file', defaultProfileImage);
    // 현재 닉네임과 소개도 함께 전송하여 정보가 누락되지 않도록 함
    ...
    // 스토어의 사용자 정보 갱신하여 변경사항 즉시 반영
    await authStore.fetchUserInfo();
    
    // 로컬 상태 업데이트
    loadUserProfile(); // 스토어에서 최신 정보를 다시 불러와 화면에 반영
    newProfilePic.value = null;
    if (fileInput.value) {
      fileInput.value.value = '';
    }

    showMessage('프로필 사진이 기본값으로 변경되었습니다.', 'success');

  } catch (error) {
    console.error("Error resetting profile picture:", error);
    showMessage('프로필 사진 변경에 실패했습니다. 다시 시도해주세요.', 'danger');
  } finally {
