          ...
          <div v-for="(group, index) in displayedGroups" :key="group.groupId" :id="`group-shelf-${group.groupId}`"
            ...
            <div class="group-shelf-title-bar">
              <div class="group-title-wrapper">
                ...
                <i v-if="group.groupId" class="bi bi-hand-index pointing-hand"></i>
              </div>
      ...
      <div class="modal-content" :class="{ 'modal-sm': isMessageBoxVisible }">
        ...
        <div v-if="isRepBookModalVisible">
          ...
          <div class="book-selection-list">
            <label v-for="book in selectableBooks" :key="book.bookId" class="book-selection-item">
...
import { bookService } from '@/services/bookService'; // bookService 임포트
...
const representBookId = ref<string | null>(null);
...
const selectableBooks = computed(() => myBooks.value.filter(book => !book.isCommunityBook));
...
async function loadUserInfo() {
  try {
    ...
    if (response.data && response.data.data) {
      ...
      representBookId.value = userData.representBookId ? String(userData.representBookId) : null;
      console.log('Representative Book ID set to:', representBookId.value);
    }
  } catch (error) {
...
async function loadMyBooks() {
  try {
    ...
    const communityBooks = communityBooksResponse.content.map((book: CommunityBook) => ({
      bookId: String(book.communityBookId), // communityBookId를 bookId로 매핑
      title: book.title,
    ...
    console.log('My books loaded:', myBooks.value.map(b => b.bookId));
    console.log('Trying to find representative book with ID:', representBookId.value);

    if (representBookId.value) {
      const repBook = myBooks.value.find(book => String(book.bookId) === String(representBookId.value));
      if (repBook) {
        console.log('Representative book found:', repBook);
        representativeBooks.value = [repBook];
      } else {
        console.log('Representative book not found in myBooks list.');
        if (myBooks.value.length > 0) {
          representativeBooks.value = [myBooks.value[0]]; // 대표책이 지정되지 않았거나 목록에 없는 경우 첫번째 책으로
        }
      }
    } else if (myBooks.value.length > 0) {
...
function selectShelfBook(book: Book | CommunityBook) {
  if (editingGroupId.value) return;

  // CommunityBook 타입 (북마크, 좋아요 목록)
  if ('communityBookId' in book) {
    router.push({ name: 'BookstoreBookDetail', params: { id: book.communityBookId } });
    return;
  }

  // Book 타입 (내 책, 그룹 책)
  if (book.isCommunityBook) {
    router.push({ name: 'BookstoreBookDetail', params: { id: book.bookId } });
  } else if (book.isGroupBook && book.groupId) {
...
async function handleChange(event: ChangeEvent, groupId: string) {
  ...
  // 커뮤니티 북인지 확인
  if (droppedBook.isCommunityBook) {
    showMessageBox('서점에 출판된 책은 그룹에 추가할 수 없습니다.', '알림');
    // UI를 원래 상태로 되돌리기 위해 그룹 목록을 다시 로드합니다.
    // 약간의 딜레이를 주어 드래그 효과가 끝난 후 리프레시되도록 합니다.
    setTimeout(() => {
      loadMyGroups();
    }, 100);
    return;
...
function removeBookFromGroup(groupId: string, bookId: string) {
  ...
  if (group) {
    ...
    if (book && confirm(`'${book.title}' 책을 '${group.groupName}' 그룹에서 삭제 하시겠습니까?`)) {
      group.books = group.books.filter(b => b.bookId !== bookId);
      showMessageBox(`책이 '${group.groupName}' 그룹에서 삭제되었습니다.`);
    }
  }
}
...
async function saveRepresentativeBookHandler() {
  if (selectedRepBookId.value) {
    console.log('Saving representative book with ID:', selectedRepBookId.value);
    const selectedBook = myBooks.value.find(book => book.bookId === selectedRepBookId.value);
    if (selectedBook) {
      try {
        console.log('Calling API to set representative book...');
        const response = await bookService.setRepresentativeBook(selectedBook.bookId, true);
        console.log('API response:', response);
        representativeBooks.value = [selectedBook];
        representBookId.value = selectedBook.bookId;
        isRepBookModalVisible.value = false;
        showMessageBox('대표책이 저장되었습니다.');
      } catch (error) {
        console.error('대표책 설정에 실패했습니다:', error);
        showMessageBox('대표책 설정에 실패했습니다.', '오류');
      }
    }
  } else {
    console.log('No representative book selected.');
  }
}
...
onMounted(async () => {
  await loadUserInfo();
  await Promise.all([loadMyBooks(), loadMyGroups()]);
});
</script>
