        ...
        <div class="editor-area" v-if="currentStory">
          <div class="editor-main">
            ...
            <div class="story-content-wrapper">
              <textarea v-model="currentStory.content" class="story-content-editor"
                :placeholder="currentBook.type === 'autobiography' ? '이곳에 이야기를 적거나 음성 녹음 시작을 누르고 말해 보세요.' : '이곳에 자유롭게 이야기를 적어보세요.'"
          ...
          <div class="editor-sidebar" :ref="el => { sidebarButtons = (el as any)?.children }">
            <template v-if="currentBook.type === 'autobiography'">
              <button @click="startAiInterview" class="btn-sidebar" :disabled="isInterviewStarted"
                :class="{ 'btn-sidebar-expanded': !isInterviewStarted }"><i class="bi bi-mic"></i> <span>AI 인터뷰
                  시작</span></button>
              <button v-if="!isRecording" @click="startRecording" class="btn-sidebar"
                :class="{ 'btn-sidebar-expanded': isInterviewStarted && !isRecording && !isContentChanged }"><i
                  class="bi bi-soundwave"></i><span>음성 답변 시작</span></button>
              <button v-else @click="stopRecording" class="btn-sidebar btn-recording"
                :class="{ 'btn-sidebar-expanded': isRecording }"><i class="bi bi-stop-circle-fill"></i> <span>음성 답변
                  완료</span></button>
              <button @click="submitAnswerAndGetFollowUp"
                :disabled="!isInterviewStarted || !isContentChanged || isSavingAnswer" class="btn-sidebar"
                :class="{ 'btn-sidebar-expanded': isInterviewStarted && isContentChanged }">
...
async function loadBookForEditing(bookId: string) {
  try {
    ...
    currentBook.value = {
      ...
      type: bookData.bookType === 'AUTO' ? 'autobiography' : bookData.bookType === 'DIARY' ? 'diary' : 'freeform',
    ...
    if (currentBook.value.stories && currentBook.value.stories.length > 0) {
      // 모든 이야기의 이미지 정보를 병렬로 가져옵니다.
      await Promise.all(
        currentBook.value.stories.map(story => story.id ? fetchEpisodeImages(story.id) : Promise.resolve())
      );
...
async function skipQuestion() {
  ...
  try {
    ...
    customAlertRef.value?.showAlert({ title: '오류', message: '마지막 질문은 건너뛸 수 없습니다.' });
  }
}
